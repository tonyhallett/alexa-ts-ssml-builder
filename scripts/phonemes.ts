...
// to be revisited
function getPhonemeDescriptionProperty(
  ipa: string,
...
function generateCode(
  localeCategorizedPhonemes: LocaleCategorizedPhonemes[]
): string {
  let code = "";
  ...
  function addPhonemesArray(
    ...
    addCode(
      `export const ${variableName} = ${JSON.stringify(phonemes)} as const;`
    );
    addCode(`export type ${variableName} = typeof ${variableName}[number];`);
  }

  function addLocalePhonemes(
    safeLocale: string,
    isIpa: boolean,
    ...
    const ipaOrXSampa = isIpa ? "IPA" : "XSampa";
    addCode(
      `export const ${safeLocale}${ipaOrXSampa}Object = ${JSON.stringify(
        localePhonemes,
        null,
        2
      )};`
  ...
  localeCategorizedPhonemes.forEach((lcp) => {
    ...
    Object.entries(lcp.categorizedPhonemes).forEach((entry) => {
      ...
      phonemes.forEach((phoneme) => {
        trimPhoneme(phoneme);
        const phonemeDescriptionProperty = getPhonemeDescriptionProperty(
          phoneme.ipa,
          phoneme.description
        );
        addLocalePhoneme(
          ipaLocalePhonemes,
          categoryProperty,
          phonemeDescriptionProperty,
          phoneme.ipa
        );
        addLocalePhoneme(
          xsampaLocalePhonemes,
          categoryProperty,
          phonemeDescriptionProperty,
          phoneme.xsampa
        );
